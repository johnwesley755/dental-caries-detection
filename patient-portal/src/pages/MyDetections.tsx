        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
             <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-3xl font-black text-slate-900 tracking-tight uppercase leading-none">Diagnostic Vault</h1>
             </div>
        ...
        {detections.length === 0 ? (
          <Card className="border-none shadow-2xl shadow-blue-900/5 rounded-[2.5rem] overflow-hidden bg-white/80 backdrop-blur-xl">
            <CardContent className="py-24 text-center space-y-6">
              ...
              <div className="space-y-2">
                <h3 className="text-2xl font-black text-slate-900 tracking-tight uppercase">No scans found</h3>
              ...
              <Button
                onClick={() => setShowUploadModal(true)}
                variant="outline"
                className="mt-4 border-2 border-slate-100 hover:bg-slate-50 rounded-[1.25rem] px-8 h-12 font-black uppercase text-[10px] tracking-widest text-slate-500"
              >
                Upload First Scan
              </Button>
            </CardContent>
                ...
                <CardContent className="p-8 space-y-6">
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Clinical Record</p>
                      <h3 className="font-black text-slate-900 tracking-tight text-lg">#{detection.detection_id.substring(0, 8)}</h3>
                    </div>
                    <Badge variant="outline" className={`px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest border-2 ${getStatusColor(detection.status)}`}>
                      {detection.status}
                    </Badge>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4 p-5 bg-slate-50 rounded-2xl border border-slate-100 shadow-inner group-hover:bg-white transition-colors duration-300">
                    <div className="space-y-1">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-tight">Teeth Analyzed</p>
                    ...
                    <div className="space-y-1 pl-4 border-l border-slate-200">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-tight">AI Findings</p>
                  ...
                  <div className="flex items-center justify-between text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50/50 px-4 py-2 rounded-xl">
                  ...
                  <Button
                    className="w-full h-12 bg-white hover:bg-primary hover:text-white text-primary border-2 border-blue-50 hover:border-primary rounded-xl font-black uppercase text-[10px] tracking-widest transition-all shadow-sm hover:shadow-xl hover:shadow-primary/20"
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300">
            <Card className="w-full max-w-xl border-none shadow-[0_40px_100px_-20px_rgba(30,58,138,0.25)] rounded-[2.5rem] overflow-hidden animate-in zoom-in-95 duration-300 bg-white">
              <div className="p-10 border-b border-slate-100 flex items-center justify-between bg-primary text-white">
                <div className="space-y-1">
                   <h2 className="text-2xl font-black tracking-tight leading-none uppercase">Clinical Upload</h2>
                   <p className="text-[10px] font-bold text-blue-100 uppercase tracking-widest">Verified Diagnostic Interface</p>
                </div>
                <button 
              ...
              <CardContent className="p-10 space-y-8">
                {!selectedFile ? (
                  <label className="flex flex-col items-center justify-center h-80 border-2 border-dashed border-slate-200 rounded-[2rem] cursor-pointer hover:border-primary hover:bg-blue-50/50 transition-all group relative overflow-hidden bg-slate-50/50">
                    ...
                    <span className="text-lg font-black text-slate-900 tracking-tight uppercase">Select Scan Image</span>
                    <span className="text-xs text-slate-400 mt-2 text-center px-10 font-bold uppercase tracking-wider leading-relaxed">
                  ...
                  <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-2 ml-1">
                         ...
                         <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Clinical Context (Optional)</label>
                      </div>
                      <textarea
                        value={notes}
                ...
                <div className="flex gap-4 pt-4">
                  <Button
                    variant="outline"
                    className="flex-1 h-14 rounded-2xl border-2 border-slate-100 text-slate-400 font-black uppercase text-[10px] tracking-widest hover:bg-slate-50 transition-all"
                    onClick={() => setShowUploadModal(false)}
                  ...
                  <Button
                    className="flex-1 h-14 bg-primary hover:bg-blue-900 text-white rounded-2xl shadow-2xl shadow-primary/20 font-black uppercase text-[10px] tracking-widest transition-all hover:scale-105 active:scale-95"
