    ...
    const spinnerContent = (
        <div className={`flex ${text ? 'flex-col' : 'inline-flex'} items-center justify-center gap-3 ${className}`}>
            <div className={`relative ${spinnerStyle.split(' ')[0]} ${spinnerStyle.split(' ')[1]}`}>
                <div className={`absolute inset-0 rounded-full border-orange-100 ${spinnerStyle.split(' ')[2]}`} />
                <div className={`absolute inset-0 rounded-full border-transparent border-t-orange-600 animate-spin ${spinnerStyle.split(' ')[2]}`} />
            </div>
