      ...
      <div className="flex items-center gap-4 flex-1">
        ...
        {title ? (
          ...
          <div className="relative w-full max-w-md hidden sm:block">
            ...
            <input 
              className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-gray-400 rounded-xl text-sm focus:ring-2 focus:ring-gray-400 transition-all placeholder:text-slate-400 font-medium" 
              placeholder="Search..." 
              type="text"
              value={searchQuery}
        ...
        >
          ...
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-primary/10 flex items-center justify-center overflow-hidden border border-primary/10 shadow-sm">
             <img 
              ...
              src={user?.profile?.profile_image_url || `https://ui-avatars.com/api/?name=${user?.full_name || 'User'}bold=true`}
            />
          </div>
        </div>
      </div>
